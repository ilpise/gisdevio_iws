...
#
#########################################################################
"""Utilities for enabling OGC WMS remote thredds in geonode."""
import json
import logging
import requests
...
# implementation of thredds catalo crawler
from .. threddsCatalog import ThreddsCatalog
...
class ThreddsWmsServiceHandler(base.ServiceHandlerBase,
                        base.CascadableServiceHandlerMixin):
    """Remote service handler for OGC WMS thredds"""

    service_type = enumerations.THREDDS

    def __init__(self, url, geonode_service_id=None):

        # modificare questo per poter far si che la url sia accettata
        # refactor base non da geonode.thredds.serviceprocessors ma da polls
    ...
    def get_cleaned_url_params(url):
        ...
        # catalog non è un parameter ma una parte della url
        # _catalog = parsed_get_args.pop('catalog') if 'catalog' in parsed_get_args else None
        ...
        print(new_url, _service, _version, _request)
        return (new_url, _service, _version, _request)

    # @staticmethod
    # def crawl_thredds(url):
    #     print('CRAWL THREDDS SERVER URL')
    #
    #     # <dataset name="TMES" ID="tmes">
    #     c = Crawl(url, select=["tmes"])
    #     print(c.datasets)
    #
    ...
    def parsed_service(self):
        print('thredds.py 137 parsed_service')
        cleaned_url, service, version, request = ThreddsWmsServiceHandler.get_cleaned_url_params(self.url)
        # _url, _parsed_service = WebMapService(
        #     cleaned_url.geturl(),
        #     version=version)
        _url, _parsed_service = ThreddsCatalog(
            cleaned_url.geturl(),
            version=version)

        print(_url, _parsed_service)
    ...
    # def crawl_thredd_service(self, owner, parent=None):
    #
    #     return 0
    def create_geonode_service(self, owner, parent=None):
        ...
        """
        # ThreddsWmsServiceHandler.crawl_thredds(self.url)
        # cleaned_url, service, version, request = ThreddsWmsServiceHandler.get_cleaned_url_params(self.url)
        #
        # print('----- create_geonode_service -----')
        # print(cleaned_url)
        # print(service)
        # print(version)
        # print(request)
...
class GeoNodeServiceHandler(ThreddsWmsServiceHandler):
    """Remote service handler for OGC WMS thredds"""
    ...
    @property
    def parsed_service(self):
        cleaned_url, service, version, request = ThreddsWmsServiceHandler.get_cleaned_url_params(self.ows_endpoint())
        _url, _parsed_service = WebMapService(
            cleaned_url.geturl(),
            version=version)
        return _parsed_service
